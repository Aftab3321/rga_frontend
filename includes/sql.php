...
/* Function for get Questions by quiz from the database
  ...
  function beginQuizProgress($data)
  {
    ...
    $_SESSION['QuizInProgressQuizID'] = (isset($progress_id)) ? $quiz_id : $progress_Quiz_info['quiz_id'];
  ...
  /* Function for updating the quiz answers
    /*--------------------------------------------------------------*/
  function insertUserAnswer($data, $selectedAnswer) {
    global $db;
    parse_str($data, $formData);
    ...
    $answer = find_by_id("answers", $option);
    $pointsEarned = $db->escape($answer['points']);
  ...
  //   }
  // }
  function updateQuizProgress($data, $selectedAnswer)
  {
    global $db; 
    insertUserAnswer($data, $selectedAnswer);
    ...
    $quizInProgressID = $db->escape($_SESSION['QuizInProgress']);
    $quizID = $db->escape($_SESSION['QuizInProgressQuizID']);
    $userID = current_user()["ID"];
    ...
    if (!$answer) {
      return ["error" => "Answer not found in database"];
  }


    $nextQuestion = $db->escape($answer['next_question']);
    $pointsEarned = $db->escape($answer['points']);

    $result = $db->query("UPDATE user_quiz_progress SET current_question = '$nextQuestion', current_question_count = (`current_question_count` + 1), answered_questions = (`answered_questions` + 1), points_earned = points_earned + '{$db->escape($pointsEarned)}' WHERE ID = '$quizInProgressID' AND user_id = '$userID' AND quiz_id = '$quizID'");
    // return ($db->affected_rows() > 0) ? ["quizInProgressID" => $quizInProgressID, "nextQuestion" => $nextQuestion] : false;
    if ($db->affected_rows() > 0) {
      return ["quizInProgressID" => $quizInProgressID, "nextQuestion" => $nextQuestion];
  } else {
      return ["error" => "Failed to update quiz progress"];
  }
  ...
  function completeQuiz()
  {
    ...
    if ($db->num_rows($result) > 0) {
      ...
      unset($_SESSION['QuizInProgressQuizID']);
...
/* Function for getting attempted quizzes from the database
  ...
  function getAttemptedQuiz()
  {
    ...
    $sql = $db->query("SELECT COUNT(DISTINCT(quiz_id)) AS quiz_attempted FROM user_quizzes WHERE user_id = '$user_id'");
    $result1 = $db->fetch_assoc($sql);
    $sql = $db->query("SELECT COUNT(DISTINCT(quiz_id)) AS quiz_attempted_this_month FROM user_quizzes WHERE user_id = '$user_id' AND MONTH(`completed_at`) = '{$db->escape($this_month)}' AND YEAR(`completed_at`) = '{$db->escape($this_year)}'");
